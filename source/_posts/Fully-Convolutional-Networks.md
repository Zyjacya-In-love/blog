...
假设图片输入为[n×n]大小，第一个卷积层输出map就为$conv1_out.size=(n-kernelsize)/stride + 1$, 记做$conv1_out.size = f(n)$, 依次类推，$conv5_out.size = f(conv5_in.size) = f(... f(n))$, 反卷积是要使$n = f‘(conv5_out.size)$成立，要确定$f'$，就需要设置deconvolution层的kernelsize，stride，padding，计算方法如下：
